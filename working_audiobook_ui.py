...
# Set page config
st.set_page_config(
    page_title="🎧 Working Audiobook Generator",
    page_icon="🎧",
    layout="wide",
...
class WorkingAudiobookGenerator:
    """Working audiobook generator with combining capability"""
    ...
    def combine_audio_segments(self, segments: List[Dict], progress_callback=None) -> bytes:
        ...
        try:
            ...
            st.warning("⚠️ FFmpeg not found. Install FFmpeg for audio combining.")
            return b""
        except Exception as e:
...
if 'generator' not in st.session_state:
    st.session_state.generator = WorkingAudiobookGenerator()

# UI Header
st.markdown('<h1 class="main-header">🎧 Working Audiobook Generator</h1>', unsafe_allow_html=True)
...
# Show options if segments are available
if 'segments' in st.session_state:
    segments = st.session_state.segments
    
    # Two columns for the main options
    col1, col2 = st.columns(2)
    
    with col1:
        st.subheader("🎧 Complete Audiobook")
        if st.button("📥 Download Combined Audiobook", type="secondary"):
            ...
            with st.spinner("🔗 Combining segments..."):
            ...
            if combined_audio:
                ...
                st.error("❌ Could not combine audiobook. Try individual segments.")
    
    with col2:
        st.subheader("📂 Individual Segments")
        with st.expander("View All Segments"):
            for i, segment in enumerate(segments):
                st.write(f"**Segment {i+1}**: {segment['emotion'].title()} ({segment['voice_style']})")
    ...
    - **📥 Combined Audiobook**: Single file with all segments (uses FFmpeg)
    - **📂 Individual Segments**: Download each segment separately
    
    The combined audiobook maintains the emotional voice changes while providing a single file for convenience.
    """)
    ...
    <li>🎤 6 different voice styles (accents/speeds)</li>
    <li>🎭 Automatic emotional voice mapping</li>
    <li>🔗 Combined audiobook generation</li>
    ...
    4. **Download** as single audiobook or individual segments
    """) 